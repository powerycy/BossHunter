...
| 简历工作室 | 从技术文档和作品材料提取带证据的事实，经人工审核后生成主简历 |
...
打开 Web Dashboard 左侧的「简历工作室」，可以：

1. 批量上传 Markdown、Word 或带文字层的 PDF 技术文档与作品材料；
2. 让 AI 从材料中提取带原文证据的候选事实；
3. 逐条修改、接受或拒绝候选事实；
4. 只使用已接受事实生成主简历草稿；
5. 预览并明确确认后，将草稿启用为后续岗位定制使用的主简历。

材料、事实和主简历版本都保存在本地 `data/` 目录。未经用户接受的事实不会进入主简历；旧简历文件不会在启用新版本时被删除。

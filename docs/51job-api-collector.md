...
1. **分布式探针（前密后疏 + 随机 + 不相邻）**
   在页码范围前半段密度高、后半段密度低地随机撒点，且任意两针不相邻。空页直接 `continue`，不触发风控。
...
5. **续采起点 / 邻位强制纳入**
   断点续采恢复后，续采起点页与其相邻页强制纳入本轮采样，保证续采区间的连续性。
...
- 探针段空页 → `continue`，计入采样但不中断。
- 风控拦截（非 JSON / status != 1）→ 走 L0-L3 分级（见第 5 节），L3 才抛 `CollectionBlockedError`。
...
|------|------|------|
| L0 | 正常 | 继续 |
| L1 | HTTP 非 200 | 记录，按速率回退重试 |
| L2 | HTTP 200 但非 JSON（如被风控页重定向为 HTML） | 记录，回退重试 |
| L3 | JSON 但 `status != 1` / 空 items（硬风控特征） | 抛 `CollectionBlockedError`，fail-closed |

设计原则：**宁可漏采，不可硬刚**。风控是账号级、累积性、单向收紧的，硬重试只会加速封禁。L3 走 fail-closed，与共享管线约定一致；L1/L2 才允许有限回退重试。
...
- `tests/test_job51_collector.py` 仍针对旧的 DOM 解析实现，需在后续 PR 中更新为覆盖 API 抓取路径与断点续采的用例。
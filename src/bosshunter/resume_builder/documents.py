"""Safe document-to-Markdown conversion shared by resume features."""
...
class ResumeUploadError(ValueError):
	"""An invalid or unsupported resume source upload."""
...
def prepare_resume_content(filename: str, content: bytes) -> tuple[str, bytes]:
	...
	return f"{Path(safe_name).stem}.md", markdown.encode("utf-8")


def pdf_to_markdown(content: bytes) -> str:
	"""Extract text-layer content from a PDF as Markdown-like text."""
	try:
